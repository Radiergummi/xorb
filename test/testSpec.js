...
describe('Testing the Xorb HTTP API', function() {
  ...
  it('Should do HEAD requests', function() {
    var headPromise = app.http.head('/test/testRunner.html').then(response => response.ok);

    return expect(headPromise).to.eventually.be.true;
  });

  it('Should do DELETE requests', function() {
    var headPromise = app.http.delete('/test/testRunner.html').then(response => response.ok);

    return expect(headPromise).to.eventually.be.true;
  });

  it('Should do POST requests', function() {
    var headPromise = app.http.post('/test/methodResponder.php').then(response => response.ok);

    return expect(headPromise).to.eventually.be.true;
  });

  it('Should do PUT requests', function() {
    var headPromise = app.http.put('/test/methodResponder.php').then(response => response.ok);

    return expect(headPromise).to.eventually.be.true;
  });

  it('Should do PATCH requests', function() {
    var headPromise = app.http.patch('/test/methodResponder.php').then(response => response.ok);
